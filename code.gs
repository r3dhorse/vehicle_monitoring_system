...
function clearTransactionLogsCache() {
  try {
    const cache = CacheService.getScriptCache();
    // Clear all transaction logs cache pages
    for (let page = 1; page <= 10; page++) {
      cache.remove(`transaction_logs_p${page}_l20`);
    }
    console.log('Transaction logs cache cleared');
  } catch (error) {
    console.warn('Could not clear transaction logs cache:', error);
  }
}
...
function logVehicleAction(data) {
  try {
    ...
    if (vehicleRow !== -1) {
      ...
      clearTransactionLogsCache(); // Clear transaction logs cache after new entry
    }
...
// Get paginated transaction logs with lazy loading support
function getTransactionLogs(page = 1, limit = 20) {
  ...
  try {
    ...
    if (!ss) {
      ...
      defaultResponse.error = 'Cannot access spreadsheet';
      return defaultResponse;
    }
    
    const logSheet = ss.getSheetByName(LOG_SHEET);
    if (!logSheet) {
      console.log('No log sheet found. Looking for sheet:', LOG_SHEET);
      defaultResponse.error = `Log sheet "${LOG_SHEET}" not found`;
      return defaultResponse;
    }
    
    // Get data range more safely
    let allData;
    try {
      allData = logSheet.getDataRange().getValues();
    } catch (dataError) {
      console.log('Error reading data range:', dataError);
      defaultResponse.error = 'Error reading sheet data';
      return defaultResponse;
    }
    
    console.log('Total rows in log sheet:', allData.length);
    
    if (!allData || allData.length <= 1) {
      console.log('No log data found (only header or empty)');
      defaultResponse.loadTime = new Date() - startTime;
      return defaultResponse;
    }
    
    // Remove header row and reverse to get newest first
    const dataRows = allData.slice(1).reverse();
    console.log('Data rows after removing header:', dataRows.length);
    ...
    console.log(`Page ${page}: startIndex=${startIndex}, endIndex=${endIndex}, pageData.length=${pageData.length}`);
    
    // Process and format the logs
    const logs = pageData.map((row, index) => {
      console.log(`Processing row ${index}:`, row);
      return {
        timestamp: row[0] || new Date(),
        plateNumber: (row[1] || 'Unknown').toString(),
        driverId: (row[2] || 'Unknown').toString(),
        action: (row[3] || 'Unknown').toString(),
        gate: (row[4] || 'Unknown').toString(),
    ...
    console.log(`Transaction logs retrieved in ${loadTime}ms:`, logs.length, 'logs');
    console.log('Final result structure:', Object.keys(result));
    
    return result;
  } catch (error) {
    console.error('Error getting transaction logs:', error);
    defaultResponse.error = error.toString();
    defaultResponse.loadTime = new Date() - startTime;
    return defaultResponse;
  }
}

// Debug function to check log sheet status
function debugLogSheet() {
  try {
    console.log('=== Log Sheet Debug ===');
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    console.log('Spreadsheet opened successfully');
    
    const logSheet = ss.getSheetByName(LOG_SHEET);
    if (!logSheet) {
      console.log('LOG_SHEET not found. Sheet name:', LOG_SHEET);
      const allSheets = ss.getSheets().map(s => s.getName());
      console.log('Available sheets:', allSheets);
      return { error: 'Log sheet not found', availableSheets: allSheets };
    }
    ...
    if (lastRow > 0) {
      ...
      console.log('Header row:', allData[0]);
      ...
      return { 
        ...
        header: allData[0],
        sampleRows: allData.slice(1, 4)
      };
    } else {
      return { success: true, totalRows: 0, message: 'Sheet is empty' };
    }
  } catch (error) {
